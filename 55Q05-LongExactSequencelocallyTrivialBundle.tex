...
\usepackage{pmmeta}
\pmcanonicalname{LongExactSequencelocallyTrivialBundle}
\pmcreated{2013-03-22 13:14:58}
\pmmodified{2013-03-22 13:14:58}
\pmowner{bwebste}{988}
\pmmodifier{bwebste}{988}
\pmtitle{long exact sequence (locally trivial bundle)}
\pmrecord{6}{33726}
\pmprivacy{1}
...
\newcommand{\Zp}{\Z/p}
\begin{document}
Let $\pi:E\to B$ is a locally trivial bundle, with fiber $F$. Then there is a long exact sequence
of homotopy groups
$$\begin{CD}
\cdots @>>>\pi_n(F)@>i_*>>\pi_n(E)@>\pi_*>>\pi_n(B)@>\partial_*>>\pi_{n-1}(F)@>>>\cdots
\end{CD}$$

Here $i_*$ is induced by the inclusion $i:F\hookrightarrow E$ as the fiber over the basepoint of $B$, and $\partial_*$ is the following
map: if $[\vp]\in\pi_n(B)$, then 
$\vp$ lifts to a map of $(D^n,\partial D^n)$ into $(E,F)$ (that is a map of the $n$-disk into $E$, taking its boundary to $F$), sending the
basepoint on the boundary to the base point of $F\subset E$.  Thus the map on $\partial D^n=S^{n-1}$, the $n-1$-sphere, defines an element of $\pi_{n-1}(F)$.  This is $\partial_*[\vp]$.  The covering homotopy property of a locally trivial bundle shows that this is well-defined.
%%%%%
%%%%%
\end{document}
