...
\usepackage{pmmeta}
\pmcanonicalname{LongExactSequenceofHomologyGroups}
\pmcreated{2013-03-22 13:14:50}
\pmmodified{2013-03-22 13:14:50}
\pmowner{mathcam}{2727}
\pmmodifier{mathcam}{2727}
\pmtitle{long exact sequence (of homology groups)}
\pmrecord{8}{33723}
\pmprivacy{1}
...
where $i_*$ is induced by the inclusion $i:(A,B)\hookrightarrow(X,B)$, $j_*$ by the inclusion
$j:(X,B)\hookrightarrow(X,A)$, and $\partial$ is the following map: given $a\in H_n(X,A)$,
choose a chain representing it.  $\partial a$ is an $(n-1)$-chain of $A$, so it represents an element
...
\end{CD}$$
where $i_\sharp$ and $j_\sharp$ are the maps on chains induced by $i$ and $j$, 
by the Snake Lemma.
%%%%%
%%%%%
\end{document}
